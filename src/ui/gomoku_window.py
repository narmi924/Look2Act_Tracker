...
class GomokuWindow(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        header = QHBoxLayout()
        self.status_label = QLabel(tx("X 落子", "X Turn"))
        ...
        self.reset_button = QPushButton(tx_button("重开", "Restart"))
        self.close_button = QPushButton(tx_button("关闭", "Close"))
    ...
    def _place_x(self, pos: tuple[int, int]) -> None:
        ...
        if self._winner_from(row, col, 1):
            self._finish_game(tx("X 获胜", "X Wins"), "🎉")
            return
        if self._is_full():
            self._finish_game(tx("平局", "Draw"), "😐")
            return
        self._place_o()
    ...
    def _place_o(self) -> None:
        ...
        if self._winner_from(row, col, 2):
            self._finish_game(tx("O 获胜", "O Wins"), "😅")
        else:
            self.status_label.setText(tx("X 落子", "X Turn"))
    ...
    def _finish_game(self, message: str, emoji: str) -> None:
        ...
        self.status_label.setText(tx(f"{message}，2秒后返回", f"{message}, returning in 2s"))
    ...
    def _on_return_tick(self) -> None:
        ...
        self.status_label.setText(tx(f"{self._result_message}，{seconds}秒后返回", f"{self._result_message}, returning in {seconds}s"))
        self.update()
    ...
    def _draw_result_overlay(self, painter: QPainter) -> None:
        ...
        painter.drawText(
            ...
            tx(f"{seconds} 秒后返回九宫格", f"Returning to launcher in {seconds}s"),
        )
